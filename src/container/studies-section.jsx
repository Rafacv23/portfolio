...
const StudiesSection = (props) => {
    return (
        <section className="section" id={props.id}>
            ...
            <ul>
                {courses.map((course) => (
                    <li id={course.id}> <img src={course.img} alt='School logo' width="64px" height="64px"></img> {course.name} - {course.school} </li>
                ))}
            </ul>
            <SubTitle className="subtitle" text="Certificates"></SubTitle>
            <ul>
                {certificates.map((certificate) => (
                    <li id={certificate.id}> <img src={certificate.img} alt='School logo' width="64px" height="64px"></img> {certificate.name} - {certificate.school} </li>
                ))}
            </ul>
        </section>
    );
